...
const EXTRA_FLAGS: Record<string, string[]> = {
  ...
  // Structured capture is dispatched before the legacy capture handler.
  capture: ['--body', '--evidence', '--project-id', '--sensitivity', '--suggested-type', '--summary', '--title', '--type', '--verification'],
  // GBrain client lifecycle commands are dispatched before engine binding.
  review: ['--action', '--confirm', '--limit', '--project-id', '--reason', '--stale', '--status', '--target', '--type', '--verification'],
  project: ['--confirmed', '--get', '--name', '--repo', '--resolved'],
  'install-client': ['--no-experience-hook'],
};
