...
## Cloud-firewall allowlist deployment

For the allowlisted server deployment, put the listener behind the cloud
firewall and a TLS termination layer, then start it with:

```bash
gbrain serve --http --port 3131 --bind 127.0.0.1 \
  --public-url https://<PUBLIC_GBRAIN_HOST> --allow-anonymous-mcp
```

`--allow-anonymous-mcp` gives firewall-allowlisted clients anonymous `read`
and `write` MCP access. It does not expose `admin` operations and does not
replace Basic Auth for `/admin` or `/admin/review`. Configure
`GBRAIN_ADMIN_BASIC_USER`, `GBRAIN_ADMIN_BASIC_PASSWORD`, and
`GBRAIN_ADMIN_ORIGIN` in a root-owned mode-600 environment file. Do not put
these values in the repository or client installer inputs.
...
on one project registry. Both clients share the Python read-only recall state machine,
which observes only bounded metadata and never calls MCP or writes GBrain.
Only high-impact decisions, explicit recall requests, and the first unexpected execution
failure arm a one-time recall token. Ordinary tasks do not. An isolated Recall Worker owns
read-only retrieval and returns only a bounded receipt envelope to the parent.
Automatic Closeout is disabled: Codex uses an error-only GBrain-managed `PostToolUse`
handler but no `Stop` handler, and OpenCode forwards only unexpected failures with no idle continuation. Manual
capture runs only when the user explicitly requests `gbrain-capture`.
