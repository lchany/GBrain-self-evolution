...
Nontrivial turns arm one one-time recall token. An isolated Recall Worker owns
read-only retrieval and returns only a bounded receipt envelope to the parent.
Automatic Closeout is disabled: Codex has no GBrain-managed `PostToolUse` or `Stop`
experience handler, and OpenCode has no tool forwarding or idle continuation. Manual
