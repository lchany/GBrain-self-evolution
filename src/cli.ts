...
export const CLI_ONLY = new Set(['init', 'reinit-pglite', 'upgrade', 'post-upgrade', 'check-update', 'integrations', 'publish', 'check-backlinks', 'lint', 'report', 'import', 'export', 'files', 'embed', 'serve', 'call', 'config', 'doctor', 'migrate', 'eval', 'sync', 'extract', 'extract-conversation-facts', 'enrich', 'features', 'autopilot', 'graph-query', 'jobs', 'agent', 'apply-migrations', 'skillpack-check', 'skillpack', 'resolvers', 'integrity', 'repair-jsonb', 'orphans', 'sources', 'mounts', 'dream', 'check-resolvable', 'routing-eval', 'skillify', 'smoke-test', 'providers', 'storage', 'repos', 'code-def', 'code-refs', 'reindex', 'reindex-code', 'reindex-frontmatter', 'code-callers', 'code-callees', 'reconcile-links', 'frontmatter', 'auth', 'friction', 'claw-test', 'book-mirror', 'takes', 'think', 'salience', 'anomalies', 'calibration', 'transcripts', 'models', 'remote', 'recall', 'forget', 'edges-backfill', 'cache', 'ze-switch', 'founder', 'brainstorm', 'lsd', 'schema', 'capture', 'review', 'project', 'install-client', 'experience-hook', 'onboard', 'conversation-parser', 'status', 'connect', 'skillopt', 'quarantine', 'self-upgrade', 'advisor', 'watch', 'reindex-search-vector']);
...
async function handleCliOnly(command: string, args: string[]) {
  ...
  if (command === 'capture') {
    const { isGbrainCaptureArgs, runGbrainCapture } = await import('./commands/gbrain-capture.ts');
    if (isGbrainCaptureArgs(args)) {
      await runGbrainCapture(args);
      return;
    }
  }

  if (command === 'review') {
    const { runGbrainReview } = await import('./commands/gbrain-review.ts');
    setCliExitVerdict(await runGbrainReview(args));
    return;
  }

  if (command === 'project') {
    const { runGbrainProject } = await import('./commands/gbrain-project.ts');
    setCliExitVerdict(await runGbrainProject(args));
    return;
  }

  if (command === 'install-client') {
    const { runInstallClient } = await import('./commands/gbrain-client-installer.ts');
    setCliExitVerdict(await runInstallClient(args));
    return;
  }

  if (command === 'experience-hook') {
    const { runExperienceHook } = await import('./commands/gbrain-experience-hook.ts');
    setCliExitVerdict(runExperienceHook(args));
    return;
  }
...
SETUP
  ...
  experience-hook <mode|status>       Control Codex experience guard mode
