...
Modes (mutually exclusive — first match wins):
  ...
  gbrain capture --title TITLE --summary BODY --evidence REF
                                  structured inbox-only draft via writer MCP
  gbrain capture retry              replay .omo/gbrain-capture/offline/*.json

Options:
  --slug SLUG          Override the default inbox/YYYY-MM-DD-<hash6> slug
  --type TYPE          Override the page type (default: note)
                       With --title, stores only a suggested inbox draft type.
  --title TITLE        Structured capture title (forces inbox-only draft mode)
  --summary BODY       Structured capture summary body
  --body BODY          Alias for --summary
