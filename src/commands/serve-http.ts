...
export async function runServeHttp(engine: BrainEngine, options: ServeHttpOptions) {
  ...
  app.use(
    ['/mcp', '/token', '/authorize', '/register', '/revoke'],
    (req: Request, res: Response, next: NextFunction) => {
      const origin = req.get('Origin');
      if (origin && !corsAllowlistOAuth?.has(origin)) {
        res.status(403).json({ error: 'origin_not_allowed', error_description: 'Origin is not allowed' });
        return;
      }
      next();
    },
    cors(corsOAuthOptions),
  );
  ...
  app.post('/token', ccRateLimiter, express.urlencoded({ extended: false }), async (req, res, next) => {
    ...
    try {
      ...
      const rawResource: unknown = req.body?.resource;
      if (rawResource !== undefined && typeof rawResource !== 'string') {
        res.status(400).json({ error: 'invalid_request', error_description: 'resource must be a URL' });
        return;
      }
      let resource: URL | undefined;
      try {
        resource = rawResource === undefined ? undefined : new URL(rawResource);
      } catch {
  ...
  const expectedMcpResource = canonicalResourceIdentifier(issuerUrl);
...
<div class="prompt">&ldquo;Give me the GBrain admin login link&rdquo;</div>
  ...
  // ---------------------------------------------------------------------------
  // Review Web UI routes (Todo 7) — mounted BEFORE the SPA fallback so
  // /admin/review/* and /admin/api/review/* are matched first. Uses the
  // same requireAdmin cookie auth as the existing admin dashboard. All
  // write operations go through the shared review core server-side; the
  // browser never sees writer credentials.
  // ---------------------------------------------------------------------------
  const { mountReviewRoutes } = await import('./serve-http-review.ts');
  mountReviewRoutes(app, engine, requireAdmin, { adminOrigin, issuerUrl });
  ...
  const rejectUnsupportedMcpMethod = (_req: Request, res: Response) => {
    res.set('Allow', 'POST');
  ...
  app.post(
    ...
    (req: Request, res: Response, next: NextFunction) => {
      ...
      let matches = false;
      try {
        matches = canonicalResourceIdentifier(tokenResource) === expectedMcpResource;
      } catch {
        matches = false;
      }
      if (!matches) {
        const message = 'Token audience does not match this MCP resource';
        res.set(
          'WWW-Authenticate',
    ...
    server.setRequestHandler(ListToolsRequestSchema, async () => {
      ...
      return { tools: buildToolDefs(mcpOperations) };
    });
