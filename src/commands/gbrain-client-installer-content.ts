...
- 用户在等待期内发送任何消息都会结束静默倒计时。正文发生变化时必须重新完整展示并重新计算 5 分钟。运行环境不支持自动等待时，不得伪造等待或写入结果；下一次获得执行机会时先核对时间和期间是否收到回复。
...
- 用户在完整预览展示后 5 分钟没有任何回复：继续调用 \`put_page\`；
- 用户拒绝或要求修改：不写入，也不离线排队；
- 用户回复含义不明确：结束静默倒计时，继续澄清，不得写入；
- 正文发生变化：重新完整展示，从展示完成时重新计算 5 分钟；
- 运行环境不支持等待后自动继续：不得伪造等待或结果；下一次获得执行机会
  时先核对是否已经超过 5 分钟，并确认期间没有收到回复。
