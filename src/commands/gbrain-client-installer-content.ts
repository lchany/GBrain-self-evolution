...
- 客户或项目规则、任务或里程碑总结、失败或根因总结，以及新增或实质更新的知识、runbook 和决策记录，在 MCP \`put_page\` 前默认需要写入前审核。用户明确免审时，免审范围默认只限当前任务、会话或项目。
- 写入前先搜索去重、自动脱敏、按固定模板整理并展示完整正文和预分类建议，同时提示：“如需拒绝或修改，请在 5 分钟内回复；5 分钟没有回复将继续写入 \`inbox/\`。”用户明确同意时立即写入；5 分钟没有任何回复时继续写入；拒绝、要求修改或含义不明的回复不得写入。
- 用户在等待期内发送任何消息都会结束静默倒计时。正文发生变化时必须重新完整展示并重新计算 5 分钟。已安装的 Codex 守卫和 OpenCode plugin 会在到期后自动恢复原会话；其他运行环境不得伪造等待或写入结果，下一次获得执行机会时先核对时间和期间是否收到回复。
...
name: gbrain-capture
description: 将经过验证、脱敏和人工审核的持久经验写入 GBrain inbox 草稿。遇到客户或项目规则、任务总结、重复故障、已验证根因、可复用流程、项目里程碑，或用户要求记录知识时使用。
---

# GBrain 经验采集

默认使用中文向用户说明召回结果、候选经验、预分类和审核状态。命令、代码、
...
向用户展示完整的脱敏正文和预分类建议，并原样告知：

> 如需拒绝或修改，请在 5 分钟内回复；5 分钟没有回复将继续写入 \`inbox/\`。

处理规则：

- 用户明确同意：立即调用 \`put_page\`；
- 用户在完整预览展示后 5 分钟没有任何回复：已安装的 Codex 守卫会自动恢复原 session；OpenCode Agent 必须在展示预览后调用 \`gbrain_experience_review_start\`，由 plugin 在 session idle 后自动恢复原 session；恢复后继续调用 \`put_page\`；
- 用户拒绝或要求修改：不写入，也不离线排队；
- 用户回复含义不明确：结束静默倒计时，继续澄清，不得写入；
- 正文发生变化：重新完整展示，从展示完成时重新计算 5 分钟；
- 未安装自动恢复组件的运行环境：不得伪造等待或结果；下一次获得执行机会时先核对是否已经超过 5 分钟，并确认期间没有收到回复。

用户可以明确免除逐条确认。除非用户明确要求全局持续生效，免审默认只限
当前任务、会话或项目。免审不取消脱敏、搜索去重、固定模板、\`inbox/\`
限制和后续晋升审核。
